...
# 使用固定阈值进行阈值化处理
thresh_value = 100  # 设置一个合适的阈值
_, thresholded_image = cv2.threshold(gradient_magnitude, thresh_value, 255, cv2.THRESH_BINARY)

# 显示原图和阈值化后的边缘检测结果
cv2.imshow("Original Image", image)
cv2.imshow("Thresholded Prewitt Edge Detection", thresholded_image)
cv2.waitKey(0)
